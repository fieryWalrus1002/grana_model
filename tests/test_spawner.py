...
class TestSpawner(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.structure_list = cls.spawner.spawn_psii(space = cls.space, batch = cls.batch)

    def test_spawner_return_type(self):
        self.assertIsInstance(self.spawner.spawn_psii(space = self.space, batch = self.batch), list)
    ...
    def test_spawn_particle_number(self):
        particle_list = self.spawner.spawn_particles(space = self.space,  k = 10, batch = self.batch)
